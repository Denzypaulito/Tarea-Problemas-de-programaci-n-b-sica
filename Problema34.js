  ...
  readline.question('Introduce la calificación de la asignatura 10: ', (calf10) => {
    ...
    let suma = 0;
      for (let i = 0; i < calf.length; i++) {
        ...
        readline.question('Introduce la calificación que quieres buscar: ', (nota) => {
            const index = calf.indexOf(Number(nota));
            if (index !== -1) {
              console.log(`La calificación ${nota} se encuentra en la asignatura ${index + 1}`);
            } else {
              console.log(`La calificación ${nota} no se encuentra en la lista`);
            }
